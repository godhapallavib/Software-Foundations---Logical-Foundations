...
Theorem plus_id_exercise : forall n m o : nat,
  n = m -> m = o -> n + m = m + o.
Proof.
  (* move all three quantifiers into the context*)
  intros n m o.
  (* move the first hypothesis into the context*)
  intros H1.
  (* move the second hypothesis into the context*)
  intros H2.
  (* rewrite the goal using first hypothesis*)
  rewrite -> H1.
  (* rewrite the goal using second hypothesis*)
  rewrite -> H2.
  reflexivity.
  Qed.
(** [] *)
...
Theorem mult_n_1 : forall p : nat,
  p * 1 = p.
Proof.
  (* move the quantifier into the context*)
  intros p. 
  (* rewrite the goal using mult_n_Sm*)
  rewrite <- mult_n_Sm.
  (* rewrite the goal using mult_n_0*)
  rewrite <- mult_n_O.
  reflexivity.
  Qed.
...
Theorem negation_fn_applied_twice:
  ...
  forall (b : bool), f (f b) = negb (negb b).
Proof.
  intros f H x.
  rewrite -> H.
  rewrite -> H.
...
Theorem lower_letter_lowers:
  forall (l : letter),
    ...
    letter_comparison (lower_letter l) l = Lt.
Proof.
  intros l. intros H. destruct l eqn:El. (* we can also introduce H in all sub cases after case analysis of l*)
  - simpl. reflexivity.
  - simpl. reflexivity.
  - simpl. reflexivity.
  - simpl. reflexivity.
