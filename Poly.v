...
Notation " x > y " := (gb x y) (at level 70) : nat_scope.

Definition filter_even_gt7 (l : list nat) : list nat := 
  filter (fun n => n > 7) (filter even l).
...
Lemma map_rev_app : forall (X Y : Type) (f : X -> Y) (l : list X) (n : X), 
...
Proof.
  ...
  - simpl. reflexivity.
  - simpl. rewrite map_rev_app. rewrite IHl'. reflexivity.
Qed.
(** [] *)
...
Definition fold_andb { X : Type} (f : X -> bool) (l : list X) : bool := 
  fold (fun x => fun y => andb (f x) y ) l true.

Example fold_andb_test1 : fold_andb even [2;4;6;8] = true.
Proof. reflexivity. Qed.

Example fold_andb_test2 : fold_andb odd [1;3;5;7;9] = true.
Proof. reflexivity. Qed.
...
(*
  ...
  - For suppose, first [l = []], we must show that 
    length [] = n -> @nth_error X [] n = None 
    Since the list is empty, n = 0. and @nth_error X [] n = None.

  - Next suppose, [l = n' :: l'] with 
    length l' = n -> @nth_error X l' n = None (the induction hypothesis).
    We must show that 
    length (n' :: l') = n -> @nth_error X (n' :: l') n = None. 

    Since length of (n' :: l') is n, there will be no element at index n.
    So, @nth_error X ( n' :: l') n = None.
*)
