...
Fixpoint oddmembers (l:natlist) : natlist :=
  match l with 
    | nil => nil 
    | h :: tl => 
        match Nat.odd h with 
          | Datatypes.true => h :: (oddmembers tl)
          | Datatypes.false => oddmembers tl
        end
  end.
  ...
  match s1 with 
    ...
    | h :: tl => 
        match count h s2 with 
          | 0 => false 
          | S _ => included tl (remove_one h s2)
        end
  end.
...
Fixpoint eqblist (l1 l2 : natlist) : bool := 
  match l1, l2 with 
    ...
    | _, nil => false 
    | h1 :: tl1, h2 :: tl2 => 
        if h1 =? h2 then eqblist tl1 tl2 else false
  end.
...
Proof.
  intros s1 s2 v. induction s1 as [| n l' IHl'].
  - simpl. reflexivity.
...
Proof.
  intros l default. destruct l. (* we don't have to use induction here, because we never use IH later. So, destruct would be enough*)
  - simpl. reflexivity.
  - simpl. reflexivity.
Qed.
(** [] *)
...
Proof.
  intros x. destruct x. 
  simpl. induction n as [| n' IHn']. (* instead of using induction, we can have a theorem proved for eqb n n = true and use it to rewrite the proof.*)
  - simpl. reflexivity.
...
(* how does this override exisiting value?? 
we're not deleting or updating anything from d above. 
we're just adding a new key value pair to the exisiting partial map.*) 

(* maps here may contain same key multiple times but with diff values. 
We consider the frst occurance of the key to get the updated value. *)
