...
(** **** Exercise: 2 stars, advanced, especially useful (add_comm_informal)
    ...
    - First, suppose [n = 0]. We must show that for any [m], 
...
Proof.
  ...
  assert (G: n + m = m + n).
    {rewrite add_comm. reflexivity. }
  simpl. rewrite H. rewrite add_assoc. rewrite G.  reflexivity.
Qed.
...
Proof.
  ...
  - assert (H: k + n' = n' + k).
    {rewrite add_comm. reflexivity. }
    simpl. rewrite add_assoc, IHn', add_assoc, H. reflexivity.
Qed.
...
Proof.
  ...
  - simpl. rewrite IHm', mul_helper. reflexivity. 
Qed.
(** [] *)
...
Proof.
  ...
  - destruct c.
    + simpl. reflexivity.
    + simpl. reflexivity.
  - simpl. reflexivity.  (* don't have to destruct here one of the or is true already*)
Qed.
...
Proof.
  ...
  - reflexivity.
  - simpl. rewrite add_comm. reflexivity. (*simpl. -->  just to see what case we're proving on the proof window. *)
...
(* This theorem fails because there can be multiple representations of binary 
   for the same natural number. For example, both Z and B0 Z represents natural
   number 0. nat_to_bin always return normalised version of binary format for a 
   natural number but there can also be leading zeros which also represent same 
   natural number (which can be our input). So, it may not return the same binary
   as given to the input. *)
...
Example normalize_example1: normalize (B0 (B0 (B0 Z))) = Z.
Proof. simpl. reflexivity. Qed.

Example normalize_example2: normalize (B0 (B1 (B0 Z))) = B0 (B1 Z).
Proof. simpl. reflexivity. Qed.

Example normalize_example3: normalize (B0 (B0 ( B1 Z))) = (B0 (B0 (B1 Z))).
Proof. simpl. reflexivity. Qed.

(* Example normalize_example4: normalize (B1 (B0 (B0 Z))) = B1 Z.
Proof. simpl. reflexivity. Qed. *)
